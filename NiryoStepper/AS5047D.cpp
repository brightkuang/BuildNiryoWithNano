...
boolean AS5047D::init_position_sensor()
{
  ...
  while (getBit(data,8)==0)
  {
    ...
    Serial.println(data);
    data=readAddress(AS5047D_CMD_DIAAGC);  Wire.beginTransmission(AS5047D_ADDRESS); 
  }
  return true;
}

//read the encoders 
boolean AS5047D::readAddress(uint16_t addr)
{
  uint16_t data;
  error=false;
...
uint16_t AS5047D::read_encode(void)
{
  uint16_t angle;
     
  angle=((uint32_t)readAddress(AS5047D_CMD_ANGLECOM))<<2; //convert the 14 bit encoder value to a 16 bit number
